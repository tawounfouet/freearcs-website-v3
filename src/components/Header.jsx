      ...
      <div className="w-full px-4 lg:px-12">
        <div className="flex justify-between items-center w-full h-20">
          ...
          <div className="flex items-center">
            {/* Logo */}
            <Link to="/" className="flex items-center mr-4 lg:mr-8" data-testid="header-logo">
              <img
                src={LOGO_URL}
              ...
              >
                {item.children ? (
                  <>
                    <Link
                      to={item.path}
                      className={`flex items-center gap-1 px-3 py-2 text-base font-medium rounded transition-colors duration-200 hover:text-primary ${
                      ...
                      <div className="absolute top-full left-0 min-w-60 bg-white shadow-lg rounded-lg border border-border py-2 z-50">
                        {item.children.map((child) => (
                          <Link
                            ...
                            className={`block px-5 py-2.5 text-base transition-colors hover:bg-muted hover:text-primary ${
                  ...
                  <Link
                    to={item.path}
                    className={`relative px-3 py-2 text-base font-medium transition-colors duration-200 hover:text-primary group ${
                      isActive(item) ? 'text-primary' : 'text-secondary'
                    }`}
                    data-testid={`nav-link-${item.path.replace('/', '')}`}
                  >
                ...
                <div key={item.path}>
                  {item.children ? (
                        ...
                        <div className="ml-4 mb-1 border-l-2 border-primary bg-muted">
                          {item.children.map((child) => (
                            <Link
                              ...
                              to={child.path}
                              onClick={closeMobile}
                              className={`block px-4 py-2.5 text-base transition-colors hover:text-primary ${
                                location.pathname === child.path
                    ...
                    <Link
                      to={item.path}
                      onClick={closeMobile}
                      className={`block px-4 py-3 text-lg font-medium transition-colors hover:text-primary ${
                        isActive(item) ? 'text-primary' : 'text-secondary'
                      }`}
